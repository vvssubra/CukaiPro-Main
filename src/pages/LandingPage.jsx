...
const TRUSTED_LOGOS = [
  { slug: 'grab',    name: 'Grab',    color: '00B14F' },
  { slug: 'airasia', name: 'AirAsia', color: 'FF0000' },
  { slug: 'shopee',  name: 'Shopee',  color: 'EE4D2D' },
  { slug: 'dhl',     name: 'DHL',     color: 'FFCC00' },
  { slug: 'samsung', name: 'Samsung', color: '1428A0' },
  { slug: 'hp',      name: 'HP',      color: '0096D6' },
  { slug: 'toyota',  name: 'Toyota',  color: 'EB0A1E' },
  { slug: 'myob',    name: 'MYOB',    color: '009BDE' },
  { slug: 'xero',    name: 'Xero',    color: '13B5EA' },
  { slug: 'siemens', name: 'Siemens', color: '009999' },
];
...
function LandingPage() {
  ...
  return (
    <div className="bg-background-dark text-white">
      ...
      <section id="trust" className="py-20 border-y border-white/10 bg-background-dark overflow-hidden relative">
        {/* Ambient glow */}
        <div className="absolute inset-0 pointer-events-none" aria-hidden="true">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[200px] rounded-full bg-emerald-500/[0.06] blur-[80px]" />
        </div>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {/* Logo marquee */}
          <div className="relative overflow-hidden rounded-2xl border border-white/[0.08] bg-white/[0.025] backdrop-blur-sm">
            <div className="absolute left-0 top-0 bottom-0 w-28 z-10 bg-gradient-to-r from-background-dark via-background-dark/80 to-transparent pointer-events-none" />
            <div className="absolute right-0 top-0 bottom-0 w-28 z-10 bg-gradient-to-l from-background-dark via-background-dark/80 to-transparent pointer-events-none" />
            ...
            <div className="py-8">
              <div className="ticker-track flex items-center gap-14 shrink-0 w-max">
                {[...TRUSTED_LOGOS, ...TRUSTED_LOGOS].map((logo, i) => (
                  ...
                  >
                    <div className="relative px-5 py-3 rounded-xl transition-all duration-300 group-hover:bg-white/[0.07] cursor-default">
                      <img
                    ...
                    <span className="text-[9px] font-semibold tracking-wider text-white/25 uppercase opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                      {logo.name}
                    </span>
                  </div>
                ))}
          ...
          <div className="flex flex-wrap items-center justify-center gap-6 sm:gap-10 mt-10">
            {[
              { icon: 'verified', value: '500+', label: 'Companies' },
              { icon: 'trending_up', value: 'RM 2.1B+', label: 'Processed' },
              { icon: 'schedule', value: '99.9%', label: 'Uptime SLA' },
              { icon: 'security', value: 'LHDN', label: 'Certified' },
            ].map((stat, i) => (
              <div key={i} className="flex items-center gap-2.5 group">
                <div className="flex items-center justify-center w-7 h-7 rounded-lg bg-emerald-500/10 border border-emerald-500/20 group-hover:bg-emerald-500/20 transition-colors duration-200">
                  <span className="material-icons-outlined text-[14px] text-emerald-400">{stat.icon}</span>
                </div>
                <div>
                  <span className="block text-sm font-bold text-white/80 leading-tight">{stat.value}</span>
                  <span className="block text-[10px] text-white/35 uppercase tracking-wider">{stat.label}</span>
                </div>
              </div>
            ))}
          </div>

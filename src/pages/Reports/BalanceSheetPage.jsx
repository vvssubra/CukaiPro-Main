...
import { BALANCE_SHEET_ORDER, BALANCE_SHEET_GROUP_LABELS } from '../../utils/accountingHelpers';
...
function BalanceSheetPage() {
  const [asOfDate, setAsOfDate] = useState(toDateStr(new Date()));
  const [reportType, setReportType] = useState('standard');
  const { accounts, balances, loading, error } = useAccountBalances(asOfDate);

  const grouped = useMemo(() => {
    const byType = {};
    BALANCE_SHEET_ORDER.forEach((type) => {
      byType[type] = (accounts || [])
        .filter((a) => a.type === type)
        .map((a) => ({ ...a, balance: balances[a.id] ?? 0 }))
        .filter((a) => Math.abs(a.balance) > 0.001 || reportType === 'comparative');
    });
    return byType;
  }, [accounts, balances, reportType]);
  ...
  const totals = useMemo(() => {
    ...
    (accounts || []).forEach((a) => {
      ...
      if (a.type === 'liability' || a.type === 'equity') totalLiabEquity += bal;
    });
    return { totalAssets, totalLiabEquity };
  }, [accounts, balances]);
    ...
    <div className="max-w-5xl mx-auto">
      <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Balance Sheet Statement</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">As at a specific date</p>
        </div>
      </div>

      <div className="mb-6 flex flex-wrap gap-4">
        <div>
          <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Reporting date</label>
          <input
            type="date"
            value={asOfDate}
      ...
      {!loading && (
        <div className="bg-white dark:bg-slate-custom rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-slate-200 dark:border-slate-700">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">As at {asOfDate}</p>
          </div>
          <div className="p-6 space-y-6">
            {BALANCE_SHEET_ORDER.map((type) => {
              const rows = grouped[type] || [];
              const label = BALANCE_SHEET_GROUP_LABELS[type] || type;
              const typeTotal = rows.reduce((s, r) => s + r.balance, 0);
              if (rows.length === 0 && reportType === 'standard') return null;
              return (
                <div key={type}>
            ...
            <div className="pt-4 border-t border-slate-200 dark:border-slate-700">
              <p className="text-sm text-slate-500 dark:text-slate-400">
                Total Assets: {formatCurrency(totals.totalAssets)} | Total Liabilities & Equity: {formatCurrency(totals.totalLiabEquity)}
              </p>
            </div>
          </div>
        </div>

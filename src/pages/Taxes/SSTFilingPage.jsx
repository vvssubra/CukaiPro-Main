...
function SSTFilingPage() {
  ...
  const [filingToSubmit, setFilingToSubmit] = useState(null);
  ...
  const handleSubmit = useCallback(
    (filing) => setFilingToSubmit(filing),
    []
  );

  const handleConfirmSubmit = useCallback(
    async () => {
      if (!filingToSubmit) return;
      setActionLoading(filingToSubmit.id);
      const result = await updateFilingStatus(filingToSubmit.id, 'submitted');
      setActionLoading(null);
      setFilingToSubmit(null);
      if (!result.success && result.error) alert(result.error);
    },
    [filingToSubmit, updateFilingStatus]
  );
  ...
  return (
    ...
    <div className="max-w-7xl mx-auto px-6 py-8">
      <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <ConfirmModal
          isOpen={!!filingToSubmit}
          onClose={() => setFilingToSubmit(null)}
          onConfirm={handleConfirmSubmit}
          title="Mark period as submitted?"
          message="This indicates you have filed SST-02 with LHDN for this period. You can still update the amount later if needed."
          confirmLabel="Mark submitted"
          cancelLabel="Cancel"
          variant="primary"
          loading={filingToSubmit ? actionLoading === filingToSubmit.id : false}
        />
    </>
  );
}
